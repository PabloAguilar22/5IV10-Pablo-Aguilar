...
function subneteo(ipv, nsubr){
    ...
    if(oct1 <= 127){
        ...
        alert("Solo se hace el subneteo de clases A, B o C >:c")
    }
...
function validarip(){
    ...
    if(ipv[0]>255 || ipv[1]>255 || ipv[2]>255 || ipv[3]>255){
        alert("Direccion IP no valida. Ingrese de nuevo. Fuera de rango D:<");
        return false;
    }else 
    ...
    if(subr>128){
        alert("El máximo de sub redes es 128 >:c");
        return false;
    }else if(ipv[0]>=192){
        if(subr >= 65){
            alert("Las clase C tienen un máximo de 64 subredes, con 7 ya no hay host");
            return false;
        }
    }else if(isNaN(subr)==true){
        alert("Número de subredes vacio. No se puede Calcular >:c");
        return false;
    }else{
...
function subneteohost(){
    ...
    if(host>8388606){
        alert("Numero de host fuera de rango. Maximo 8,388,606")
        return false;
    }else if(isNaN(host)==true){
        alert("Numero de host vacio. No se puede Calcular >:c");
        return false;
    }
    var oct1 = ipv[0];  
    var bitr = 0;   
