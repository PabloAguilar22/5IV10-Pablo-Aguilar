function prueba() {
	...
	if (document.getElementById('subred').value == "") {
		alert("El número de subredes no puede ser 0");
		return false;
	}
	...
	var conC = 0;
	//var bandera = false;
	//console.log("LAPTM: " + subr);
	while (con < subr) {
		
		//console.log("Valor antes: " + con);
		con = Math.pow(2,bits);
		//bandera = (con <= subr);
		//console.log("Valor después: " + con + " Bandera: " + bandera);
		bits++;
	...
	for (let index = 0; index < ipp.length; index++) {
		...
		//prueba[index] = parseInt(ar[index], 2);
		//console.log("Ar: " + ar[index]);	
	}
	...
	//console.log("IP en decimal: " + xde);

	//calculo de rango de subred
	...
	cont = 0;
    conC = 0;
	tabla1.innerHTML = "<thead><td class='td_admon'>N. de subred</td><td class='td_admon'>IP inicial</td><td class='td_admon'>IP inicial en binario</td><td class='td_admon'>IP final</td><td class='td_admon'>IP final en binario</td><td class='td_admon'>Hosts disponibles</td></thead>";
